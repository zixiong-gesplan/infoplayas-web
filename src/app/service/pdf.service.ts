  ...
  beachs.forEach( beach => {
    ...
    this.doc.autoTable({
      ...
      body:incidents.map( report =>{
        ...
        return [report.id, "--","--","--",report.alarmSender,report.date + " " + report.date,report.type,report.alarmSender,resources,report.numberLifeGuard,report.affectedPeopleNumber,report.hourCall112,report.cecoes,report.delayArriveExternalResources,evacuacion,traslado]
      }),
    ...
    var body =afectados.map(persona =>{
      ...
      return [persona["incident_id"],persona["hora"],persona['dia'],persona["type_intervention"],genero,persona["country"],persona["age"],persona["municipality"],substances,persona["profile"],persona["emergencyGrade"],traslado,primeros_auxilios,alta_voluntaria,desa]
    })
    if( body.length >0){
      this.doc.addPage()
    ...
    afectados.forEach( person =>{
      ...
      if(person.age< 20){ ageStatics["10-20"] = ageStatics["10-20"]++ ||1}
      if(person.age< 35){ ageStatics["20-35"] = ageStatics["20-35"]++ ||1}
      if(person.age< 50){ ageStatics["35-50"] = ageStatics["35-50"]++ ||1}
      if(person.age< 60){ ageStatics["50-60"] = ageStatics["50-60"]++ ||1}
