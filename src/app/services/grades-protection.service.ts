...
export class GradesProtectionService {
    ...
    calculate(parentid: string, token: string) {
        ...
        relationsIds.forEach(value => {
            params.append('relationshipId', value);
        ...
        forkJoin(httpRequests).subscribe(results => {
            ...
            const incidents_Sports = (results[0] as any).relatedRecordGroups[0].relatedRecords[0];
            const vIncidents = incidents_Sports.attributes.incidentes_mgraves > 0 ? 5 :
                incidents_Sports.attributes.incidentes_graves > 0 ? 3 : 0;
            ...
            const env_sea = (results[1] as any).relatedRecordGroups[0].relatedRecords[0];
            ...
            this.records.forEach(value => {
                value.grado = this.calculateGradeLvl(value.afluencia, vIncidents, vSports, vSeaConditions, vEnvironment,
                    vPopulation);
            });
